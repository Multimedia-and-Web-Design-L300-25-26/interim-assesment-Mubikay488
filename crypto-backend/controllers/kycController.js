...
const getKycStatus = async (req, res) => {
  try {
    const doc = await KycDocument.findOne({ userId: req.user._id }).sort({
      createdAt: -1,
    });
    res.json({
...
const uploadId = async (req, res) => {
  try {
    ...
    const frontImageUrl = files.front
      ? `/uploads/kyc/${files.front[0].filename}`
      : undefined;
    const backImageUrl = files.back
      ? `/uploads/kyc/${files.back[0].filename}`
      : undefined;
    ...
    res
      .status(201)
      .json({ message: "ID uploaded successfully", document: doc });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
...
const submitKyc = async (req, res) => {
  try {
    ...
    if (!doc)
      return res
        .status(400)
        .json({ message: "No KYC documents found. Upload ID first." });
